...
<script>
function chkAll (all) {
//전체선택 체크박스 클릭 시 모든체크박스에 대한 체크여부를 처리하는 함수
...
function cartDel(ocidx) {	// 장바구니내의 특정 상품을 삭제하는 함수
...
function getSelectedChk() { // 체크박스들중 선택된 체크박스들의 값 (value)들을 쉼표로 구분하여 문자열로 리턴하는 함수
	var chk = document.frmCart.chk;
	var idxs = ""; //chk컨드롤 배열에서 선택된 체크박스의 값들을 누적해서 저장할 변수
...
function chkDel(){	// 사용자가 선택한 상품(들)을 삭제하는 함수
	...
	//선택한 체크박스의 oc_idx값들이 쉼표르 기준으로 '1,2,3,4'형태로 문자열로 저장됨
...
<h2>장바구니</h2>
<form name="frmCart" action="orderForm" method="post"><!-- 구매를 위한 action -->
<input type="hidden" name="kind" value="c">
<input type="checkbox" name="all" id="all" checked="checked" onclick="chkAll(this);"><label for="all">전체선택</label>
<input type="hidden" name="chk">  <!-- chk 체크박스를 배열로 처리하기 위해 인위적으로 지정해 놓은 컨트롤 -->
<% 
if (cartList.size() > 0){ //장바구니에 담긴 상품이 있을경우
	int total = 0; //총 수매가격의 누적값을 저장할 변수
...
<%
	int max = 10; // 수량변경을 위한 최대값을 지정하는 함수
...
%>
	<option value="<%=ps.getPs_idx() %>" <%=slt %>> <%=txt %>
<% 	
		}
	}
%>
...
<td width="10%">
	<select name="cnt" onchange="cartUp(<%=ocidx %>, 0, this.value);"><!-- 선택된 ocidx(t_order_cart에서 oc_idx), 0 ,선택된 값(값만 필요하므로this.value)  -->
<%
		for (int i = 1 ; i <= max; i++) {
			String slt = "";
...
<td width="15%"><%=oc.getPi_price() * oc.getOc_cnt() %></td>
<td width="*">
